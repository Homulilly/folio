...
- **sharp 管线** `services/convert.ts` 的 `convertFile`:据 `ConvertOptions` 应用 per-format 参数(质量/渐进/压缩/effort/bitdepth)+ `keepExif()`/`keepIccProfile()` + `keepAlpha:false→flatten`;输出 = `targetDir ?? 原目录` + 换扩展名;冲突 skip/overwrite/number(md5_compare 当 number)。**只写新文件,绝不覆盖原图**——`intended===源文件`(原地同扩展名)强制 `suggestExportPath` 改名。sharp 0.35 是 **N-API,Electron 下零 rebuild**;async API 自带 libvips 线程,故**直接跑主进程,不需要 Worker**(M6 spike 已验证)。**注意:预置 libvips/libheif 不带 HEVC 解码器、也没编 JXL**——`sharp.format.heif.input=true` 只是容器识别,真解 HEVC-HEIC 会抛 `Support for this compression format has not been built in`;JXL 直接 `input=false`。HEIC/JXL 预览靠下方「缓存层」的 sips 兜底(macOS),Windows 无解(`MVP 不输出` 这些格式)。**JXL 显示也不能靠 Chromium**:实测 Electron 42(Chromium 148)`<img>` 解不了 JXL(默认 + `enable-features=JpegXl` 都失败),故走和 HEIC 一样的 sips 转码。
...
- **统一解码层 `services/decode.ts`(仅 macOS)**:sharp 解码失败时,缩略图/预览(`thumbnail.ts`)、尺寸(`imageInfo.ts`)、转换(`convert.ts`)统一调 `decodeToPng`/`osDimensions` → `sips -s format png` / `sips -g`(`execFile` 传参数组无 shell 注入),转临时 PNG 后走同一条 webp 管线,完后删临时文件。**按嗅探格式白名单门控** `OS_DECODABLE_FORMATS = {heic, heif, jxl, bmp}`(用 magic-byte 格式而非扩展名,故 `.jpg` 伪装的 HEIC 也走)。**ICO 刻意不在内**:ICO 不做 magic-byte 嗅探(`format=null`),故不经 sips——ICO 仍由 Chromium 显示(original 变体),但不作为转换输入。**非 darwin 直接 false**(Windows/Linux 的 djxl/libheif 是 Phase 2/3)。
...
- **真实尺寸** `services/imageInfo.ts` 的 `imageDimensions(path)`:sharp `metadata()` 只读文件头(不全解码)+ EXIF 方向校正,mtime 内存缓存;IPC `image:dimensions`。供「大图策略」判断尺寸、给非可解码格式喂正确 `setNatural`。**`decode.ts` 的 `osDimensions` 兜底(仅 macOS,白名单 `{heic,heif,jxl,bmp}`)**:大 HEIC 是分块网格,预置 libheif 在 metadata 阶段就拒绝(`Security limit exceeded: Number of references in iref box exceeds 16`),JXL/BMP 则 libvips 根本不解,`metadata()` 抛错 → 退回 `sips -g pixelWidth -g pixelHeight`。**没有这条,单图拿不到 `naturalWidth` 就无法缩放/fit**(预览仍由 sips 兜底正常显示,故症状是「能看不能缩放」)。
