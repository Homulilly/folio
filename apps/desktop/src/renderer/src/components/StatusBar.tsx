...
const MODE_LABELS = { single: 'Single', dual: 'Dual', triple: 'Triple', quad: 'Quad' } as const
...
export function StatusBar(): React.JSX.Element {
  ...
  return (
    <div className="flex h-7 flex-none items-center gap-3.5 border-t border-white/[0.06] bg-[#161618] px-3.5 font-mono text-[11px] text-[rgba(235,235,245,0.5)]">
      {item ? (
        <>
          {mode !== 'single' && <span className="text-[#0A84FF]">{MODE_LABELS[mode]}</span>}
          <span className="truncate text-[rgba(235,235,245,0.75)]">{item.fileName}</span>
          <span>{dims}</span>
          <span>{formatLabel(item)}</span>
          <span>{formatBytes(item.size)}</span>
          <div className="ml-auto flex items-center gap-3.5">
            {mode !== 'single' && syncZoom && <span className="text-[#0A84FF]">Sync</span>}
