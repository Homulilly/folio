...
- **sharp 管线** `services/convert.ts` 的 `convertFile`:据 `ConvertOptions` 应用 per-format 参数(质量/渐进/压缩/effort/bitdepth)+ `keepExif()`/`keepIccProfile()` + `keepAlpha:false→flatten`;输出 = `targetDir ?? 原目录` + 换扩展名;冲突 skip/overwrite/number(md5_compare 当 number)。**只写新文件,绝不覆盖原图**——`intended===源文件`(原地同扩展名)强制 `suggestExportPath` 改名。sharp 0.35 是 **N-API,Electron 下零 rebuild**;async API 自带 libvips 线程,故**直接跑主进程,不需要 Worker**(M6 spike 已验证)。**注意:预置 libvips/libheif 不带 HEVC 解码器、也没编 JXL**——`sharp.format.heif.input=true` 只是容器识别,真解 HEVC-HEIC 会抛 `Support for this compression format has not been built in`;JXL 直接 `input=false`。HEIC 预览靠下方「缓存层」的 sips 兜底,JXL 当前无解(`MVP 不输出` 这些格式)。
